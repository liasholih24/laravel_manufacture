  ...
  <style>
    ...
    <div class="ibox-title row">
        <ol class="breadcrumb col-sm-6 col-xs-12" style="font-size: 14px; padding-top: 6px; ">
          ...
          <?php if(Sentinel::getUser()->hasAccess(['templates.create'])): ?>
          ...
          <select class="select2_demo_1 form-control" style="max-width:275px;margin-right: 10px;" onchange="if (this.value) window.location.href=this.value">
						<option value="<?php echo e(url('templates')); ?>">Select Template</option>
						<?php $__currentLoopData = $filters; $__env->addLoop($__currentLoopData); foreach($__currentLoopData as $filter): $__env->incrementLoopIndices(); $loop = $__env->getLastLoop(); ?>
						<option value="<?php echo e(url('templates/' . $filter->id . '/filter')); ?>" <?php if ($filter->id == $id) echo ' selected="selected"'; ?>>
...
<tbody>
  ...
  <tr>
      ...
      <td>
        <?php if(Sentinel::getUser()->hasAccess(['templates.show'])): ?>
        <a href="<?php echo e(url('templates/' . $table->id . '')); ?>" class="btn btn-primary btn-outline btn-xs">View</a>
        <?php endif; ?>
        <?php if(Sentinel::getUser()->hasAccess(['templates.edit'])): ?>
        <a href="<?php echo e(url('templates/' . $table->id . '/edit')); ?>" class="btn btn-outline btn-primary btn-xs">Edit</a>
        <?php endif; ?>
