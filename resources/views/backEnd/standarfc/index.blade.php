...
<tbody>
<?php $i = 0 ?>
@foreach($standarfc as $item)
<?php $i++ ?>
    <tr>
        <td>{{ $item->id }}</td>
        <td>{{ $item->umur0 }} - {{ $item->umur1 }}</td>
        <td>{{ $item->fc0 }} - {{ $item->fc1 }}</td>
        <td>
            <a href="{{ url('standarfc/' . $item->id . '/edit') }}" class="btn btn-outline btn-primary btn-xs">Update</a>
            {!! Form::open([
                'method'=>'DELETE',
                'url' => ['standarfc', $item->id],
